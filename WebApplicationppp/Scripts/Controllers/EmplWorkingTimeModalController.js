myApp.controller('EmplWorkingTimeModalController', ['$scope', '$modalInstance', 'items', 'SalonsService',
    function ($scope, $modalInstance, items, SalonsService) {

        console.log(items);
        $scope.items = items;
        $scope.itemsForUse = angular.copy(items.EmplSchedule);
        $scope.itemsForReturn = angular.copy(items.EmplSchedule);
        var date = new Date();
        date.setHours(10);
        date.setMinutes(0);
        ...
        $scope.ok = function () {
            ...
            SalonsService.updateEmplWorkTime($scope.items.EmployeeID, $scope.itemsForReturn).then(function (data) {
                console.log(data);
